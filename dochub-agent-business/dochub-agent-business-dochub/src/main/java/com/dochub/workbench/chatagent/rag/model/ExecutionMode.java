...
public enum ExecutionMode {
    ...
    /**
     * 计划-执行（Plan-and-Execute）模式。
     *
     * <p>适用于需要多步骤推理、对比分析、方案设计等复杂开放式问题。该模式由
     * {@code PlanAndExecuteExecutor} 执行：先由 Planner 把目标拆成若干执行步骤，
     * 每个步骤交给带工具的 ReAct Agent 自主执行，最后统一汇总成完整回答。步骤数与
     * 每步的模型调用都受预算护栏约束，避免死循环烧 token。</p>
     */
    PLAN_AND_EXECUTE,
