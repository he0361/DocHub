...
import com.dochub.workbench.manage.model.es.DocumentKeywordIndexRecord;
...
public class ElasticsearchDocumentKeywordSearchGateway implements DocumentKeywordSearchGateway {
    ...
    @Override
    public void indexChunks(List<DochubDocumentChunk> chunkList) {
        if (CollUtil.isEmpty(chunkList)) {
            return;
        }

        Map<Long, DochubDocument> documentMap = loadDocumentMap(chunkList);
        BulkRequest.Builder bulkBuilder = new BulkRequest.Builder()
            .index(properties.getElasticsearch().getIndexName())
            .refresh(Refresh.WaitFor);

        for (DochubDocumentChunk chunk : chunkList) {
            DochubDocument document = documentMap.get(chunk.getDocumentId());
    ...
    private Map<Long, DochubDocument> loadDocumentMap(List<DochubDocumentChunk> chunkList) {
        List<Long> documentIds = chunkList.stream()
            .map(DochubDocumentChunk::getDocumentId)
        ...
        List<DochubDocument> documents = documentMapper.selectBatchIds(documentIds);
        Map<Long, DochubDocument> documentMap = new LinkedHashMap<>();
    ...
    private DocumentKeywordIndexRecord toIndexRecord(DochubDocumentChunk chunk, DochubDocument document) {
