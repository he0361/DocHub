...
import com.dochub.workbench.chatagent.data.DochubChatExchangeTraceStage;
import com.dochub.workbench.chatagent.mapper.DochubChatExchangeTraceStageMapper;
...
public class MybatisConversationTraceStageStore implements ConversationTraceStageStore {
    ...
    private final DochubChatExchangeTraceStageMapper traceStageMapper;
    private final ObjectMapper objectMapper;

    @Resource
    private UidGenerator uidGenerator;

    public MybatisConversationTraceStageStore(DochubChatExchangeTraceStageMapper traceStageMapper,
    ...
    public long startStage(String conversationId,
        ...
        DochubChatExchangeTraceStage stage = new DochubChatExchangeTraceStage();
        stage.setId(stageId);
    ...
    public void finishStage(long stageId,
        ...
        traceStageMapper.update(
            null,
            new LambdaUpdateWrapper<DochubChatExchangeTraceStage>()
                .eq(DochubChatExchangeTraceStage::getId, stageId)
                .set(DochubChatExchangeTraceStage::getStageState, stageState.getCode())
                .set(DochubChatExchangeTraceStage::getEndTime, new Date())
                .set(DochubChatExchangeTraceStage::getDurationMs, Math.max(durationMs, 0L))
                .set(DochubChatExchangeTraceStage::getSummaryText, StrUtil.blankToDefault(summaryText, ""))
                .set(DochubChatExchangeTraceStage::getErrorMessage, StrUtil.blankToDefault(errorMessage, ""))
                .set(DochubChatExchangeTraceStage::getSnapshotJson, writeNullableJson(snapshot))
        );
    }
    ...
    public List<ConversationTraceStageView> listStageViews(String conversationId, long exchangeId) {
        return traceStageMapper.selectList(
                new LambdaQueryWrapper<DochubChatExchangeTraceStage>()
                    .eq(DochubChatExchangeTraceStage::getConversationId, conversationId)
                    .eq(DochubChatExchangeTraceStage::getExchangeId, exchangeId)
                    .orderByAsc(DochubChatExchangeTraceStage::getStageOrder)
                    .orderByAsc(DochubChatExchangeTraceStage::getStartTime)
                    .orderByAsc(DochubChatExchangeTraceStage::getId)
            )
            .stream()
            .map(this::toView)
            .toList();
    ...
    private ConversationTraceStageView toView(DochubChatExchangeTraceStage stage) {
