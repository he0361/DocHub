...
public class DocumentManageController {
    ...
    @Operation(summary = "查询索引构建实时进度")
    @PostMapping("/index/progress/query")
    public ApiResponse<DocumentIndexBuildProgressVo> queryIndexBuildProgress(@RequestBody DocumentDetailQueryDto dto) {
        return ApiResponse.ok(documentManageService.queryIndexBuildProgress(dto.getDocumentId()));
    }
