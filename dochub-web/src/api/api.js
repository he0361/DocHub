...
const ADMIN_TOKEN_KEY = 'dochub-admin-token'
const ADMIN_USER_KEY = 'dochub-admin-user'
...
export const adminAuthApi = {
  ...
  currentProfile() {
    // 后端 /admin/auth/me 是 GET，必须走 requestJson，否则 405 导致权限 profile 拉取失败
...
export const adminUserApi = {
  list() {
    return requestApiEnvelope('/admin/user/list', { method: 'POST' })
  },

  save(payload) {
    return requestApiEnvelope('/admin/user/save', {
      method: 'POST',
  ...
  status(payload) {
    return requestApiEnvelope('/admin/user/status', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  }
...
export const manageApi = {
  ...
  queryIndexBuildProgress(payload) {
    return requestApiEnvelope('/manage/document/index/progress/query', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
...
/**
 * 文枢 DocHub 文档工作台接口：模板中心 + 文档生成 + 导出 + 一键入库。
 */
export const docgenApi = {
  templatePage(payload = {}) {
    return requestApiEnvelope('/manage/workbench/template/page', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
  ...
  templateDetail(payload) {
    return requestApiEnvelope('/manage/workbench/template/detail', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
  ...
  templateSave(payload) {
    return requestApiEnvelope('/manage/workbench/template/save', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },

  templateDelete(payload) {
    return requestApiEnvelope('/manage/workbench/template/delete', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
  ...
  generate(payload) {
    return requestApiEnvelope('/manage/workbench/docgen/generate', {
      method: 'POST',
      body: stringifyManageValue(payload),
      timeout: 300000
    })
  ...
  /** 参考文档仿写：上传参考文档 + 需求，仿照格式生成 */
  generateReference({ file, requirement, referenceDocumentId }) {
    const formData = new FormData()
    if (file) {
      formData.append('file', file)
    }
    ...
    formData.append('meta', new Blob([JSON.stringify(meta)], { type: 'application/json' }))
    return requestMultipartApiEnvelope('/manage/workbench/docgen/generate-reference', formData, { timeout: 300000 })
  },

  /**
   * 参考文档仿写（SSE 流式）：正文逐字输出，不阻塞。
   * onEvent 收到 { type: 'status' | 'text' | 'done' | 'error', content }。
   */
  generateReferenceStream({ file, requirement, referenceDocumentId, onEvent }) {
    const formData = new FormData()
    if (file) {
      formData.append('file', file)
    }
    ...
    return new Promise((resolve, reject) => {
      const controller = new AbortController()
      fetch(buildApiUrl('/manage/workbench/docgen/generate-reference/stream'), {
        method: 'POST',
        headers: {
          ...buildAuthHeaders()
        },
  ...
  records(payload = {}) {
    return requestApiEnvelope('/manage/workbench/docgen/records', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
  ...
  deleteRecord(payload) {
    return requestApiEnvelope('/manage/workbench/docgen/record/delete', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
  ...
  ingest(payload) {
    return requestApiEnvelope('/manage/workbench/docgen/ingest', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },

  exportDocument(recordCode, format) {
    const controller = new AbortController()
    ...
    return fetch(buildApiUrl(`/manage/workbench/docgen/export?${params.toString()}`), {
      method: 'GET',
      headers: {
        ...buildAuthHeaders()
      },
      ...
      .then((response) => {
        ...
        if (!response.ok) {
          handleUnauthorized(response)
          return readResponseMessage(response).then((message) => {
      ...
      .then((blob) => {
        const fileName = `${String(recordCode || 'doc')}.${format || 'md'}`
...
/**
 * 文枢 DocHub 技能列表接口（市场 / 已装管理）。
 */
export const skillApi = {
  marketList(payload = {}) {
    return requestApiEnvelope('/manage/workbench/skill/market/list', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
  ...
  marketDetail(payload) {
    return requestApiEnvelope('/manage/workbench/skill/market/detail', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
  ...
  install(payload) {
    return requestApiEnvelope('/manage/workbench/skill/market/install', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },

  installedList(payload = {}) {
    return requestApiEnvelope('/manage/workbench/skill/installed/list', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
  ...
  enable(payload) {
    return requestApiEnvelope('/manage/workbench/skill/installed/enable', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
  ...
  disable(payload) {
    return requestApiEnvelope('/manage/workbench/skill/installed/disable', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
  ...
  deleteSkill(payload) {
    return requestApiEnvelope('/manage/workbench/skill/installed/delete', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },

  /** 上传技能包（SKILL.md 或 .zip），解析入库并启用 */
  uploadSkill(file) {
    const formData = new FormData()
    formData.append('file', file)
    return requestMultipartApiEnvelope('/manage/workbench/skill/installed/upload', formData)
  },

  /** 校验管理员密码（用于查看技能存放位置） */
  verifyAdmin(payload) {
    return requestApiEnvelope('/manage/workbench/skill/installed/verify-admin', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  },
  ...
  usage(payload = {}) {
    return requestApiEnvelope('/manage/workbench/skill/installed/usage', {
      method: 'POST',
      body: stringifyManageValue(payload)
    })
  }
