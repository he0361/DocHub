...
import { saveAdminAuth, saveAdminProfile } from '../utils/adminAuth'
...
async function submitLogin() {
  ...
  try {
    ...
    // 拉取账号信息与权限
    try {
      const profile = await adminAuthApi.currentProfile()
      saveAdminProfile(profile || {})
    } catch (e) {
      // token 已下发但 profile 拉取失败时，保留空 profile（菜单按无权限展示）
      saveAdminProfile({})
    }
