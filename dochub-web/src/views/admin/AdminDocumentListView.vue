          ...
          <table class="document-table">
            ...
            <tbody>
              <tr v-for="item in documents" :key="item.documentId" class="document-table-row">
                ...
                <td class="document-cell">
                  <div v-if="docProgress(item)" class="doc-progress">
                    <!-- 索引构建中：五段式实时进度 -->
                    <div v-if="hasCode(item.indexStatus, 2)" class="doc-build">
                      <div class="doc-build-segments" :title="buildProgressMessage(item)">
                        <span
                          v-for="seg in buildSegmentStates(item)"
                          :key="seg.code"
                          class="doc-build-segment"
                          :class="seg.state"
                          :title="seg.label"
                        >
                          <span class="doc-build-segment-fill" :style="{ width: seg.fillPct + '%' }"></span>
                        </span>
                      </div>
                      <span class="doc-progress-label" :class="{ error: docFailed(item) }">{{ buildProgressMessage(item) }}</span>
                    </div>
                    <!-- 其他处理状态：原有进度条 -->
                    <template v-else>
                      <div class="doc-progress-bar">
                        <div class="doc-progress-fill" :class="{ error: docFailed(item) }" :style="{ width: docProgress(item).percent + '%' }"></div>
                      </div>
                      <span class="doc-progress-label" :class="{ error: docFailed(item) }">{{ docProgress(item).label }}</span>
                    </template>
                  </div>
                  <span v-else class="doc-progress-done" :class="{ error: docFailed(item) }">{{ docFinalState(item) }}</span>
                </td>
...
import { formatDateTime, formatFileSize, hasCode, normalizeCode } from '../../utils/manageFormat'
...
async function loadDocuments(page = currentPage.value, silent = false) {
  // silent=true 用于进度轮询：不闪加载态，静默更新进度，用户无感
  if (!silent) {
...
function hasRunningDocumentTask(item) {
  // 只把"正在执行"（RUNNING）的任务视为不可删；NEW（排队/卡住）、失败、成功均允许删除。
  // 与后端 deleteDocument 保持一致：后端只拦截 RUNNING 任务，
  // 避免"索引构建中但任务已卡死/失败"的文档因 indexStatus=2 永远无法删除。
  return hasCode(item?.latestTaskStatus, 2)
}
...
// ===== 解析/索引进度条 =====
const STAGE_PERCENT = { 1: 10, 2: 30, 3: 45, 4: 50, 5: 60, 6: 67, 7: 70, 8: 88, 9: 100 }

function docFailed(item) {
  return hasCode(item?.parseStatus, 4) || hasCode(item?.indexStatus, 4)
}

function docProgress(item) {
  if (!item?.documentId) {
    return null
  }
  const stage = Number(item.currentStage) || 0
  // 解析中
  if (hasCode(item.parseStatus, 2)) {
    return { percent: STAGE_PERCENT[stage] || 10, label: item.currentStageName || '解析中', active: true }
  }
  // 排队待解析
  if (hasCode(item.parseStatus, 1) && hasCode(item.latestTaskStatus, 1)) {
    return { percent: 5, label: '排队解析中', active: true }
  }
  // 策略已推荐，等待用户确认
  if (hasCode(item.strategyStatus, 2)) {
    return { percent: 50, label: '待确认切块策略', active: true }
  }
  // 索引构建中
  if (hasCode(item.indexStatus, 2)) {
    return { percent: STAGE_PERCENT[stage] || 60, label: item.currentStageName || '索引构建中', active: true }
  }
  // 策略确认后待索引构建
  if (hasCode(item.strategyStatus, 3) && hasCode(item.indexStatus, 1)) {
    return { percent: 55, label: '待索引构建', active: true }
  }
  return null
}

function docFinalState(item) {
  if (docFailed(item)) {
    return '处理失败'
  }
  if (hasCode(item.parseStatus, 3) && hasCode(item.strategyStatus, 3) && hasCode(item.indexStatus, 3)) {
    return '已完成'
  }
  return '-'
}

function hasInProgressDocument() {
  return documents.value.some((item) => docProgress(item))
}

// ===== 索引构建分段实时进度 =====
const buildProgressMap = ref({})

// 各构建阶段的整体进度区间（与后端 DocumentIndexBuildProgressService 保持一致）
const BUILD_STAGE_SEQUENCE = ['5', '6', '7', '8', '9']
const BUILD_STAGE_RANGES = {
  5: [60, 67],
  6: [67, 70],
  7: [70, 88],
  8: [88, 95],
...
const BUILD_STAGE_LABELS = { 5: '切块执行', 6: '切块后处理', 7: '向量化', 8: '关键词索引', 9: '入库完成' }

function buildProgressOf(item) {
  const progress = buildProgressMap.value[String(item?.documentId)]
  return progress && !progress.failed ? progress : null
}

function buildSegmentStates(item) {
  const progress = buildProgressOf(item)
  const stageCode = normalizeCode(progress?.stageCode)
  const percent = Number(progress?.percent) || 0
  const stageIndex = BUILD_STAGE_SEQUENCE.indexOf(stageCode)
  return BUILD_STAGE_SEQUENCE.map((code, index) => {
    const range = BUILD_STAGE_RANGES[code] || [0, 100]
    let fillPct = 0
    let state = 'pending'
    if (index < stageIndex) {
      fillPct = 100
      ...
      fillPct = Math.round(Math.max(0, Math.min(1, (percent - range[0]) / span)) * 100)
    }
    return { code, label: BUILD_STAGE_LABELS[code] || code, state, fillPct }
  })
}
...
function buildProgressMessage(item) {
  const progress = buildProgressMap.value[String(item?.documentId)]
  if (progress?.failed) {
    return progress.message || '索引构建失败'
  }
  if (progress) {
    const stageLabel = BUILD_STAGE_LABELS[normalizeCode(progress.stageCode)] || progress.stageName || '索引构建中'
    const percentText = Number(progress.percent) || 0
    return progress.message
      ? `${stageLabel} · ${percentText}% · ${progress.message}`
      : `${stageLabel} · ${percentText}%`
  }
  return docProgress(item)?.label || '索引构建中'
}

async function refreshBuildProgress() {
  const buildingDocs = documents.value.filter((item) => hasCode(item?.indexStatus, 2))
  if (!buildingDocs.length) {
    buildProgressMap.value = {}
    return
  }
  const nextMap = { ...buildProgressMap.value }
  await Promise.all(buildingDocs.map(async (item) => {
    try {
      const data = await manageApi.queryIndexBuildProgress({ documentId: item.documentId })
      if (data) {
        nextMap[String(item.documentId)] = data
      }
      else {
        delete nextMap[String(item.documentId)]
      }
    }
    catch (error) {
      // 静默失败：保留上一次进度，等待下次轮询重试
    }
  }))
  buildProgressMap.value = nextMap
}

// ===== 轮询：存在处理中/排队文档时，自动刷新列表 =====
let pollTimer = null

function startPolling() {
  stopPolling()
  pollTimer = setInterval(() => {
    if (hasInProgressDocument() && !listLoading.value && !deletingDocumentId.value) {
      loadDocuments(currentPage.value, true)
    }
  }, 3000)
}
...
.doc-progress-label.error,
.doc-progress-done.error {
...
.doc-build-segment.completed .doc-build-segment-fill {
  background: var(--color-primary);
}

.doc-build-segment.current .doc-build-segment-fill {
  animation: doc-build-pulse 1.4s ease-in-out infinite;
}

@keyframes doc-build-pulse {
  0%, 100% { opacity: 1; }
  50% { opacity: 0.5; }
}
