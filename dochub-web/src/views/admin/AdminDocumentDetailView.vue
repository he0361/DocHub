          ...
          <div v-if="showBuildTracker" ref="buildTrackerRef" class="build-progress-card build-progress-card-inline">
            <div class="build-progress-header">
              ...
              <div class="build-progress-meta">
                <span v-if="buildLiveProgress && !buildLiveProgress.failed" class="build-progress-percent">{{ buildLiveProgress.percent || 0 }}%</span>
...
const BUILD_STAGE_LIBRARY = [
  ...
  { code: '7', order: '03', label: '向量化', description: '生成 embedding 并写入 Qdrant' },
  { code: '8', order: '04', label: '关键词索引', description: '同步 chunk 关键词到 Elasticsearch' },
  { code: '9', order: '05', label: '入库完成', description: '回写状态并将本次索引标记为可用' }
]
...
const buildLiveProgress = ref(null)
...
const buildTrackerDescription = computed(() => {
  ...
  const live = buildLiveProgress.value
  if (live && !live.failed) {
    const liveStage = BUILD_STAGE_LIBRARY.find((item) => item.code === normalizeCode(live.stageCode))
    const stageLabel = liveStage?.label || live.stageName || ''
    const percent = Number(live.percent) || 0
    return live.message
      ? `实时进度：${stageLabel} ${percent}% · ${live.message}，系统持续轮询中。`
      : `实时进度：${stageLabel} ${percent}%，系统持续轮询中。`
  }
  if (live?.failed) {
    return live.message || '索引构建执行失败。'
  }
...
const buildStageItems = computed(() => {
  ...
  return BUILD_STAGE_LIBRARY.map((stage, index) => {
    ...
    const live = buildLiveProgress.value
    const description = status === 'current' && live && !live.failed && live.message
      ? live.message
      : stage.description
    return { ...stage, description, status, statusLabel }
  })
})
...
async function loadIndexBuildProgress() {
  try {
    const data = await manageApi.queryIndexBuildProgress({ documentId: documentId.value })
    buildLiveProgress.value = data || null
  } catch (error) {
...
.build-progress-meta { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
